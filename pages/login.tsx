...
const Login: NextPageWithLayout<Props> = ({ authProviders }) => {
    ...
    return (
        <form className="space-y-6" method="post" action="/api/login" onSubmit={handleLogin}>
            ...
            {Object.values(authProviders).some((p) => p) ? (
                <>
                    <hr className="border-dashed" />

                    <div className="text-gray flex flex-col items-center space-y-3">
                        <p className="my-0">Or login with</p>
                        <a className="flex items-center space-x-2" href="/api/auth/github">
                            <svg
...
export async function getServerSideProps(context: GetServerSidePropsContext): Promise<GetServerSidePropsResult<Props>> {
    ...
    return {
        props: {
            ...
            user,
            authProviders: {
                github: Boolean(process.env.GITHUB_CLIENT_ID) && Boolean(process.env.GITHUB_CLIENT_SECRET),
                google: false,
                gitlab: false,
            },
        },
